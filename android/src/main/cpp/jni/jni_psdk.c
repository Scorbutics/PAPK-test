...
JNIEXPORT jint JNICALL Java_com_scorbutics_rubyvm_RubyVMNative_00024Companion_exec(JNIEnv* env, jobject clazz, jstring scriptContent, jstring fifoLogs, jstring fifoCommands, jstring fifoReturn,
    ...
    //TODO fifoLogs_c, fifoCommands_c, fifoReturn_c
    const int result = ExecMainRubyVM(scriptContent_c, -1, -1,
                                      rubyDirectoryPath_c, executionLocation_c,
                                      nativeLibsDirLocation_c, additionalParam_c);
...
Java_com_scorbutics_rubyvm_RubyVMNative_00024Companion_updateVmLocation(JNIEnv *env, jobject thiz, jstring executionLocation, jstring additionalParams) {
    (void) thiz;
    ...
    setenv("RUBY_VM_ADDITIONAL_PARAM", additionalParams_c == NULL ? "" : additionalParams_c, 1);
...
int StartGameFromNativeActivity(ANativeActivity* activity) {
    ...
    LoggingSetNativeLoggingFunction(__android_log_write);
    //TODO open outputFilename
    const int result = ExecMainRubyVM(startScriptContent, -1, -1,
                                      rubyBaseDirectory,
                                      executionLocation,
